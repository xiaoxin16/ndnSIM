...
main(int argc, char* argv[])
{
  ...
  topologyReader.SetFileName("src/ndnSIM/examples/topologies/topo-tree.txt"); //g_CP.getTopoFilePath()
  topologyReader.Read ();
  ...
  for (NodeList::Iterator node = NodeList::Begin(); node != NodeList::End(); node++) {
	  ...
	  if(name.find("leaf-1")!=string::npos||name.find("leaf-2")!=string::npos||name.find("leaf-3")!=string::npos||name.find("leaf-4")!=string::npos)
	  {
		  consumerNodes.Add ( *node );
	  }
  }    
  producerNodes.Add (Names::Find<Node> ("root")); 
...
/*   ndn::AppHelper consumerHelper(g_CP.getConsumerType());
  ...
  consumerHelper.SetAttribute("Frequency", StringValue(g_CP.getFrequency()));
  ...
  ndn::PitTracer::InstallAll("/usr/local/src/trace/pit-trace.txt", Seconds(0.1));
